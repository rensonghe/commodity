...
min = 40
slip = 1
data = pd.read_csv('/home/xianglake/songhe/%s/%s/tick_factor/%s_tick_factor_%s_%smin.csv'%(exchange, future, future, dollar, min))
...
def time_interval(data):
    ...
    end_time4 = datetime.datetime.strptime('00:00:00','%H:%M:%S').time()
...
data = data.set_index('datetime')
#%%
def ATR(data, rolling=10):
    high = data['price'].rolling(rolling).max()
    low = data['price'].rolling(rolling).min()
    price_high = abs(high - data['price'].shift(rolling))
    price_low = abs(low - data['price'].shift(rolling))
    high_low = high - low
    data_concat = pd.concat([price_high, price_low, high_low], axis=1)
    TR = data_concat.max(axis=1)
    ATR = pd.Series(TR, index=data.index).rolling(rolling).mean()
    return ATR, high, low
#%%
bar = 15
x = 0.001
...
#%%
time_1 = '2023-02-06 09:00:00'
time_2 = '2023-04-02 03:00:00'
...
# df_1 = predict.loc[predict['predict']>np.percentile(y_pred_train[-1000000:], 90)]
# df_0 = predict.loc[predict['predict']<np.percentile(y_pred_train[-1000000:], 10)]
print(len(df_1))
print(len(df_0))
df_1['side'] = 'buy'
df_0['side'] = 'sell'
...
# signal_1 = signal[signal['predict']>=np.percentile(y_pred_train[-1000000:], 90)]
# signal_0 = signal[signal['predict']<=np.percentile(y_pred_train[-1000000:], 10)]
...
signal_df_only = signal_df.loc[:,[vwapv, 'price','predict','target','side']]
def abs_classify(y):
...
for out_threshold in out_threshold_set:
    ...
    final_df_ = signal_df_[signal_df_['out'] >= np.percentile(secondary_y_pred_train[-10000:], out_threshold)]
    # final_df_ = sec_predict_[sec_predict_['out_pctrank'] >= out_threshold]
    # final_df_ = sec_predict_[sec_predict_['out'] >= np.percentile(secondary_y_pred_train[-300000:], out_threshold)]
    ...
    final_df.to_csv('/home/xianglake/songhe/future_backtest/{}/DCE_{}_20230206_0331_{}bar_{}_ST1.0_20230926_filter_{}_{}_{}.csv'
                    .format(future, future, bar, vwapv, out_threshold, x, dollar))
    # final_df.to_csv(
    #     '/home/xianglake/songhe/future_backtest/{}/SHFE_{}_20230605_0729_{}bar_{}_ST1.0_20230915_pctrank_{}_{}_{}.csv'
    #     .format(future, future, bar, vwapv,out_threshold, x, dollar))


#%%
out_threshold_set = [60,70,80]
# out_threshold_set = [0.5,0.6,0.7,0.8,0.9]
for out_threshold in out_threshold_set:
    # out_threshold = 70
    ...
    df2 = pd.read_csv('/home/xianglake/songhe/future_backtest/{}/SHFE_{}_20230206_0331_{}bar_{}_ST1.0_20230907_pctrank_{}_{}_{}.csv'.format(future, future, bar, vwapv, out_threshold, x, dollar))
    df3 = pd.read_csv('/home/xianglake/songhe/future_backtest/{}/SHFE_{}_20230403_0603_{}bar_{}_ST1.0_20230907_pctrank_{}_{}_{}.csv'.format(future, future, bar, vwapv, out_threshold, x, dollar))
    df4 = pd.read_csv('/home/xianglake/songhe/future_backtest/{}/SHFE_{}_20230605_0729_{}bar_{}_ST1.0_20230907_pctrank_{}_{}_{}.csv'.format(future, future, bar, vwapv, out_threshold, x, dollar))
    ...
    final_df.to_csv('/home/xianglake/songhe/future_backtest/{}/SHFE_{}_20230206_0729_{}bar_{}_ST1.0_20230907_pctrank_{}_{}_{}.csv'.format(future, future, bar, vwapv,out_threshold, x, dollar))
