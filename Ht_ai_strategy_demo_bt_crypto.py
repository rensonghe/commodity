    ...
    def on_tick_new(self, newTick:TickData):
        ...
        if match_signal is not None:
            ...
            if side == 'sell' and curPos > 0:
                ...
                self.sell(price=newTick.ask_price_1, volume=curPos, closetime=newTick.closetime)
                msg_ = f'下空单平多仓---平仓价格:{newTick.ask_price_1}---size:{curPos}---time:{current_time}---symbol:{self.__code__}'
                self.log_text(msg_)
            ...
            if side == 'buy' and curPos < 0:
                ...
                self.log_text(msg_)
                # print('---------------------------下空单平多仓-----------------------',time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(newTick.closetime)), '品种:',self.__code__)
                self.buy(price=newTick.bid_price_1, volume=-curPos, closetime=newTick.closetime)
                msg_ = f'下多单平空仓---平仓价格:{newTick.bid_price_1}---size:{curPos}---time:{current_time}---symbol:{self.__code__}'
                self.log_text(msg_)
