...
min = 120
slip = 1
data = pd.read_csv('/home/xianglake/songhe/%s/%s/tick_factor/%s_tick_factor_%s_%smin_%s.csv'%(exchange, future, future, dollar,min, 22_23))
# data_2021 = pd.read_csv('/home/xianglake/songhe/%s/%s/tick_factor/%s_tick_factor3_%s_%smin_21.csv'%(exchange, future, future, dollar, min))
# data_2022 = pd.read_csv('/home/xianglake/songhe/%s/%s/tick_factor/%s_tick_factor3_%s_%smin_22.csv'%(exchange, future, future, dollar, min))
# data_2023 = pd.read_csv('/home/xianglake/songhe/%s/%s/tick_factor/%s_tick_factor3_%s_%smin_23.csv'%(exchange, future, future, dollar, min))
...
def time_interval(data):
    ...
    start_time = datetime.datetime.strptime('09:31:00', '%H:%M:%S').time()
    end_time = datetime.datetime.strptime('11:30:00','%H:%M:%S').time()

    start_time2 = datetime.datetime.strptime('13:00:00', '%H:%M:%S').time()
    end_time2 = datetime.datetime.strptime('14:45:00','%H:%M:%S').time()
...
train_set_ = train_set_.iloc[:,7:110] #65
train_target_ = train_["target"]

train_set = train[train_col]
train_set = train_set.iloc[:,7:110] #65
train_target = train["target"]

test_set = test[train_col]
test_set = test_set.iloc[:,7:110]
...
test_set_ = test_set_.iloc[:,7:110]
...
def LGB_bayesian(learning_rate, num_leaves, bagging_fraction, feature_fraction, min_child_weight, min_child_samples,
    ...
    # weight = [0.03, 0.05, 0.07, 0.09, 0.11, 0.2, 0.45]
...
def lightgbm_model(X_train, X_train_target,X_train_, X_train_target_, X_test, X_test_target, X_test_, X_test_target_, LGB_BO):
    ...
    features = train_.iloc[:, 7:110].columns
...
# train_predict = train_predict.iloc[-50000:,:]
...
#     return ranks[-1]
# #
# final_predict['pctrank'] = final_predict['predict'].rolling(50000).apply(pctrank, engine='numba', raw=True)
# # final_predict['datetime'] = pd.to_datetime(final_predict['closetime']+28800000, unit='ms')
# final_predict = final_predict[(final_predict.datetime >= time_1)&(final_predict.datetime <= time_2)]
# #
# # df_1 = final_predict.loc[final_predict['pctrank']>=0.95]
# # df_0 = final_predict.loc[final_predict['pctrank']<=0.05]
#
...
# signal_1 = signal[signal['predict']>=np.percentile(y_pred_train[-30000:], 90)]
# signal_0 = signal[signal['predict']<=np.percentile(y_pred_train[-30000:], 10)]
# signal['pctrank'] = final_predict['pctrank']
# signal_1 = signal[signal['pctrank']>0.95]
# signal_0 = signal[signal['pctrank']<0.05]
print(len(signal_1))
print(len(signal_0))
signal_1['side'] = 'buy'
signal_0['side'] = 'sell'
...
# print(signal_df.loc[:,['return','pctrank']].corr())
...
#%%
signal_df_only_ = signal_df_only.reset_index()
signal_df_only_ = signal_df_only_.set_index('datetime').groupby(pd.Grouper(freq='2min')).apply('first')
signal_df_only_ = signal_df_only_.dropna()
signal_df_only_ = signal_df_only_.reset_index(drop=True)
signal_df_only_['datetime'] = pd.to_datetime(signal_df_only_['closetime']+28800000, unit='ms')
signal_df_only_ = signal_df_only_.set_index('datetime')
# signal_df_only_['p'] = np.where(signal_df_only_['side']=='buy',1,0)
print(signal_df_only_.loc[:,['target','predict']].corr())
signal_df_only = signal_df_only_.copy()
#%%
signal_df_only['closetime'] = signal_df_only['closetime'].astype('int64')
# signal_df_only.to_csv(
#     '/home/xianglake/songhe/future_backtest/{}/CFFEX_{}_20230206_0331_{}bar_{}_ST2.0_20231102_filter_80_{}_{}_{}.csv'
#     .format(future, future, bar, vwapv,0, x, dollar))
# signal_df_only.to_csv(
#     '/home/xianglake/songhe/future_backtest/{}/CFFEX_{}_20230403_0603_{}bar_{}_ST2.0_20231102_filter_80_{}_{}_{}.csv'
#     .format(future, future, bar, vwapv,0, x, dollar))
# signal_df_only.to_csv(
#     '/home/xianglake/songhe/future_backtest/{}/CFFEX_{}_20230605_0729_{}bar_{}_ST2.0_20231102_filter_80_{}_{}_{}.csv'
#     .format(future, future, bar, vwapv,0, x, dollar))
signal_df_only.to_csv(
    '/home/xianglake/songhe/future_backtest/{}/CFFEX_{}_20230731_0923_{}bar_{}_ST2.0_20231102_filter_80_{}_{}_{}.csv'
    .format(future, future, bar, vwapv,0, x, dollar))
# signal_df_only.to_csv(
#     '/home/xianglake/songhe/future_backtest/{}/CFFEX_{}_20230925_1118_{}bar_{}_ST2.0_20231102_filter_80_{}_{}_{}.csv'
#     .format(future, future, bar, vwapv,0, x, dollar))
...
for out_threshold in out_threshold_set:
    ...
    final_df.to_csv(
        '/home/xianglake/songhe/future_backtest/{}/CFFEX_{}_20230731_0923_{}bar_{}_ST1.0_20231031_filter_90_{}_{}_{}.csv'
...
#     df2 = pd.read_csv('/home/xianglake/songhe/future_backtest/{}/CFFEX_{}_20230206_0331_{}bar_{}_ST2.0_20231025_filter_80_{}_{}_{}.csv'.format(future, future, bar, vwapv, out_threshold, x, dollar))
#     df3 = pd.read_csv('/home/xianglake/songhe/future_backtest/{}/CFFEX_{}_20230403_0603_{}bar_{}_ST2.0_20231025_filter_80_{}_{}_{}.csv'.format(future, future, bar, vwapv, out_threshold, x, dollar))
#     df4 = pd.read_csv('/home/xianglake/songhe/future_backtest/{}/CFFEX_{}_20230605_0729_{}bar_{}_ST2.0_20231025_filter_80_{}_{}_{}.csv'.format(future, future, bar, vwapv, out_threshold, x, dollar))
#     # df5 = pd.read_csv('/home/xianglake/songhe/future_backtest/{}/CFFEX_{}_20230731_0923_{}bar_{}_ST1.0_20231012_filter_{}_{}_{}.csv'.format(
...
#     final_df.to_csv('/home/xianglake/songhe/future_backtest/{}/CFFEX_{}_20230206_0729_{}bar_{}_ST2.0_20231025_filter_80_{}_{}_{}.csv'.format(future, future, bar, vwapv,out_threshold, x, dollar))
...
#%%
df2 = pd.read_csv('/home/xianglake/songhe/future_backtest/{}/CFFEX_{}_20230206_0331_{}bar_{}_ST2.0_20231102_filter_80_{}_{}_{}.csv'.format(future, future, bar, vwapv, 0, x, dollar))
df3 = pd.read_csv('/home/xianglake/songhe/future_backtest/{}/CFFEX_{}_20230403_0603_{}bar_{}_ST2.0_20231102_filter_80_{}_{}_{}.csv'.format(future, future, bar, vwapv, 0, x, dollar))
df4 = pd.read_csv('/home/xianglake/songhe/future_backtest/{}/CFFEX_{}_20230605_0729_{}bar_{}_ST2.0_20231102_filter_80_{}_{}_{}.csv'.format(future, future, bar, vwapv, 0, x, dollar))
df5 = pd.read_csv('/home/xianglake/songhe/future_backtest/{}/CFFEX_{}_20230731_0923_{}bar_{}_ST2.0_20231102_filter_80_{}_{}_{}.csv'.format(
...
print(final_df.loc[:, ['target', 'predict']].corr())
# print(final_df.loc[:, ['target', 'pctrank']].corr())
final_df.to_csv('/home/xianglake/songhe/future_backtest/{}/CFFEX_{}_20230206_0923_{}bar_{}_ST2.0_20231102_filter_80_{}_{}_{}.csv'.format(future, future, bar, vwapv,0, x, dollar))
