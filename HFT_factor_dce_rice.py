...
def calc_wap1(depth, trade):
    wap = (depth['bid_price1'] * depth['ask_size1'] + depth['ask_price1'] * depth['bid_size1']) / (depth['bid_size1'] + depth['ask_size1'])
    return wap
...
def price_impact(depth, trade, level=1):
...
def avg_turnover(depth, trade, rolling=500):
    return depth[['ask_size1', 'bid_size1']].sum(axis=1)
...
def _bid_withdraws_volume(l, n, levels=1):
    withdraws = 0
    for price_index in range(2, 2 + 1 * levels, 1):
        now_p = n[price_index]
        for price_last_index in range(2, 2 + 1 * levels, 1):
            if l[price_last_index] == now_p:
                withdraws -= min(n[price_index + 1] - l[price_last_index + 1], 0)

    return withdraws

def _ask_withdraws_volume(l, n, levels=1):
    withdraws = 0
    for price_index in range(0, 1 * levels, 1):
        now_p = n[price_index]
        for price_last_index in range(0, 1 * levels, 1):
            if l[price_last_index] == now_p:
                withdraws -= min(n[price_index + 1] - l[price_last_index + 1], 0)
...
def cal_weight_volume(depth):
    ...
    w = np.array(w) / sum(w)
    wb = depth['bid_size1'] * w[0]
    wa = depth['ask_size1'] * w[0]
    return wb, wa
...
def buying_amplification_ratio(depth, trade, rolling):
    ...
    :return:
    '''
    biding = depth['bid_size1']*depth['bid_price1']
    asking = depth['ask_size1']*depth['ask_price1']
...
def buying_willing_stength(depth, trade, rolling):
    biding = (depth['bid_size1'])
    asking = (depth['ask_size1'])
    positive_buy = np.where(trade['price'] >= depth['ask_price1'].shift(1), trade['amount']-trade['amount'].shift(1), 0)
    positive_sell = np.where(trade['price'] <= depth['bid_price1'].shift(1), trade['amount']-trade['amount'].shift(1), 0)
...
def add_factor_process(depth, trade, futuresize=10, min=40):
    ...
    df['ask_price1'] = depth.loc[:, ['ask_price1']]
    df['bid_price1'] = depth.loc[:, ['bid_price1']]
    ...
    df['inf_ratio'] = inf_ratio(depth=None, trade=trade, rolling=60*10)
    ...
    df['price_impact'] = price_impact(depth=depth, trade=None, level=1)
    ...
    df['price_idxmax'] = price_idxmax(depth=depth, trade=None, rolling=120 * min)
    df['center_deri_two'] = center_deri_two(depth=depth, trade=None, rolling=120 * min)
    ...
    df['weighted_price_to_mid'] = weighted_price_to_mid(depth=depth, trade=None, levels=1, alpha=1)
    ...
    df['positive_buying'], df['positive_buying_oi_change'] = positive_buying(depth=depth, trade=trade,
                                                                             rolling=120 * min)
    df['positive_selling'], df['positive_selling_oi_change'] = positive_selling(depth=depth, trade=trade,
                                                                                rolling=120 * min)
    ...
    df['buy_price_bias_level1'], df['buy_amount_agg_ratio_level1'], df[
        'buy_oi_agg_ratio_level1'] = buy_order_aggressivenes_level1(depth=depth, trade=trade, rolling=120 * min,
                                                                    futuresize=futuresize)
    df['buy_price_bias_level2'], df['buy_amount_agg_ratio_level2'], df[
        'buy_oi_agg_ratio_level2'] = buy_order_aggressivenes_level2(depth=depth, trade=trade, rolling=120 * min,
                                                                    futuresize=futuresize)
    df['sell_price_bias_level1'], df['sell_amount_agg_ratio_level1'], df[
        'sell_oi_agg_ratio_level1'] = sell_order_aggressivenes_level1(depth=depth, trade=trade, rolling=120 * min,
                                                                      futuresize=futuresize)
    df['sell_price_bias_level2'], df['sell_amount_agg_ratio_level2'], df[
        'sell_oi_agg_ratio_level2'] = sell_order_aggressivenes_level2(depth=depth, trade=trade, rolling=120 * min,
                                                                      futuresize=futuresize)
    ...
    df['game'], df['game_buy_oi'], df['game_sell_oi'] = game(depth=depth, trade=trade, rolling=120 * min,
                                                             futuresize=futuresize)
    df['active_buying'], df['t_active_buying'], df['norm_active_buying'], df[
        'confi_norm_active_buying'] = multi_active_buying(depth=depth, trade=trade, rolling=120 * min)
    df['active_selling'], df['t_active_selling'], df['norm_active_selling'], df[
        'confi_norm_active_selling'] = multi_active_selling(depth=depth, trade=trade, rolling=120 * min)
    df['HCVOL'], df['HCP'], df['LCVOL'], df['LCP'], df['small_HCVOL'], df['small_HCP'], df['small_LCVOL'], df[
        'small_LCP'] = regret_factor(depth=depth, trade=trade, rolling=120 * min, futuresize=futuresize)
    df['high_price_volume_ratio_80'], df['low_price_volume_ratio_20'], df['high_price_volume_ratio_90'], df[
        'low_price_volume_ratio_10'] = high_low_price_volume_ratio(depth=depth, trade=trade, rolling=120 * min)
    df['ht_trend_str_close'], df['ht_trend_str_avg'] = hf_trend_str(depth=depth, trade=trade, rolling=120 * min,
                                                                    futuresize=futuresize)
    df['oi_rank_1'], df['oi_rank_5'], df['oi_rank_10'], df['oi_rank_20'] = oi_rank(depth=depth, trade=trade,
                                                                                   rolling=120 * min)
