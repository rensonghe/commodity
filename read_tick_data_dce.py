...
import datetime
from HFT_factor_dce import *
# year = 2022
future = 'I'
futuresize = 100
exchange = 'dce'
dollar = [3_500_000,4_000_000,2_500_000,3_000_000]
# dollar = [1_500_000,1_000_000,2_500_000,2_000_000]
...
all_data = all_data.rename({'timestamp':'closetime','last':'price','vol':'volume'}, axis='columns')
...
#%%
all_data['avgprice'] = all_data['amount']/all_data['volume']/futuresize
all_data['size'] = np.where((all_data['avgprice'] > all_data['ask_price1'].shift(1)), all_data['volume']-all_data['volume'].shift(1),np.where((all_data['avgprice'] < all_data['bid_price1'].shift(1)),(-1)*(all_data['volume']-all_data['volume'].shift(1)),
                                                                                                                                              np.where(all_data['bid_price1']>all_data['bid_price1'].shift(1), all_data['volume']-all_data['volume'].shift(1),(-1)*(all_data['volume']-all_data['volume'].shift(1)) )))
#
...
trade = data.loc[:, ['closetime', 'price', 'volume', 'amount', 'size','high', 'low']]
...
# dce
final_data['vwapv_15s'] = (final_data['price']*final_data['volume']).rolling(4*15).sum()/final_data['volume'].rolling(4*15).sum()
final_data['vwapv_5s'] = (final_data['price']*final_data['volume']).rolling(4*5).sum()/final_data['volume'].rolling(4*5).sum()
final_data['vwapv_30s'] = (final_data['price']*final_data['volume']).rolling(4*30).sum()/final_data['volume'].rolling(4*30).sum()
...
for i in dollar:
    ...
    data = dollar_bar_df(final_data, 'amount',i)
    data.to_csv('/home/xianglake/songhe/%s/%s/tick_factor/%s_tick_factor_%s.csv'%(exchange,future, future, i))
# data = data.set_index('datetime')
