...
def flow_amount(trade, depth, rolling):
    flow_amount = ((trade['amount']-trade['amount'].shift(1))*((trade['price']-trade['price'].shift(1))))/abs(trade['price']-trade['price'].shift(1))
...
def add_factor_process(depth, trade, futuresize=10):
    ...
    df['mpb_5min'] = mpb_5min(depth=depth, trade=trade,rolling=120*5,futuresize=futuresize)
    ...
    df['buy_price_bias_level1'], df['buy_amount_agg_ratio_level1'] = buy_order_aggressivenes_level1(depth=depth,trade=trade,rolling=1000,futuresize=futuresize)
    df['buy_price_bias_level2'], df['buy_amount_agg_ratio_level2'] = buy_order_aggressivenes_level2(depth=depth,trade=trade,rolling=1000,futuresize=futuresize)
    df['sell_price_bias_level1'], df['sell_amount_agg_ratio_level1'] = sell_order_aggressivenes_level1(depth=depth,trade=trade,rolling=1000,futuresize=futuresize)
    df['sell_price_bias_level2'], df['sell_amount_agg_ratio_level2'] = sell_order_aggressivenes_level2(depth=depth,trade=trade,rolling=1000,futuresize=futuresize)
    ...
    df['game'] = game(depth=depth, trade=trade, rolling=120*10,futuresize=futuresize)
    df['flow_amount'] = flow_amount(depth=depth, trade=trade, rolling=120*10)
    df['active_buying'], df['t_active_buying'], df['norm_active_buying'], df['confi_norm_active_buying'] = multi_active_buying(depth=depth, trade=trade, rolling=120*10)
    df['active_selling'], df['t_active_selling'], df['norm_active_selling'], df['confi_norm_active_selling'] = multi_active_selling(depth=depth, trade=trade, rolling=120*10)
    df['HCVOL'], df['HCP'], df['LCVOL'], df['LCP'] = regret_factor(depth=depth, trade=trade, rolling=120*10,futuresize=futuresize)
    return df
