...
class Ht_ai_strategy_demo_bt(CtaTemplate):
    ...
    def on_tick_new(self, newTick: TickData):
        ...
        amount = (self.capital / self.deposit_rate) * self.pos_rate * 1 / 2

        if int(closetime_ / 1000) - int(self.kill_time / 1000) > 60 * 2:
            # 多仓止盈止损
            if self.get_position(self.__code__) > 0 and abs(curPos * self.get_position_avgpx(self.__code__)) * self.futuresize > amount:
                ...
                if con1 == 1:
                    ...
                    self.sell(price=newTick.bid_price_1, volume=self.get_position(self.__code__) *1/2,
            ...
            if self.get_position(self.__code__) < 0 and abs(curPos * self.get_position_avgpx(self.__code__)) * self.futuresize > amount:
                ...
                if con1 == 1:
                    ...
                    self.cancel_all(self.__code__)
                    self.kill_time = closetime_ / 1000
                    self.buy(price=newTick.ask_price_1, volume=-self.get_position(self.__code__) *1/2,
                             closetime=closetime_)
        ...
        if match_signal is not None:
            ...
            slip = 1
            price = round(vwapv / self.pricetick) * self.pricetick
            # print('vwapv:',vwapv)
            # print(price)
            ...
            place_value = (self.capital / self.deposit_rate) * self.pos_rate / self.split_count  # 挂单金额
            buy_size = round(place_value / (newTick.ask_price_1 * self.futuresize), 8)  # 买单量
            sell_size = round(place_value / (newTick.bid_price_1 * self.futuresize), 8)  # 卖单量
            max_limited_order_value = (self.capital / self.deposit_rate) * self.pos_rate  # 最大挂单金额
            ...
            for key in ordict.keys():
                limit_orders_values += abs(float(ordict[key].price)) * abs(float(ordict[key].leftQty)) * self.futuresize
            # 持仓金额+挂单金额
            final_values = limit_orders_values + (
                        abs(self.get_position_avgpx(self.__code__)) * abs(curPos)) * self.futuresize
            ...
            if side == 'sell' and curPos > 0:
                ...
                self.sell(price=price - self.pricetick * slip, volume=curPos, closetime=newTick.closetime)
                msg_ = f'下空单平多仓---平仓价格:{price - self.pricetick * slip}---size:{curPos}---time:{current_time}---symbol:{self.__code__}'
                self.log_text(msg_)
            ...
            if side == 'buy' and curPos < 0:
                ...
                self.log_text(msg_)
                # print('---------------------------下空单平多仓-----------------------',time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(newTick.closetime)), '品种:',self.__code__)
                self.buy(price=price + self.pricetick * slip, volume=-curPos, closetime=newTick.closetime)
                msg_ = f'下多单平空仓---平仓价格:{price + self.pricetick * slip}---size:{curPos}---time:{current_time}---symbol:{self.__code__}'
                self.log_text(msg_)
            ...
            if side == 'sell' and position_value >= -self.pos_rate * self.capital * (
                ...
                if max_limited_order_value <= final_values * 1.0001 and curPos == 0:
                    ...
                    msg_ = f'空单超出挂单金额--撤单'
                    self.log_text(msg_)
                self.sell(price=price - self.pricetick * slip, volume=sell_size, closetime=newTick.closetime)
                msg_ = f'开空仓---开仓价格:{price - self.pricetick * slip}---size:{-sell_size}---time:{current_time}---symbol:{self.__code__}'
                self.log_text(msg_)
            ...
            if side == 'buy' and position_value <= self.pos_rate * self.capital * (
                ...
                if max_limited_order_value <= final_values * 1.0001 and curPos == 0:
                    ...
                    msg_ = f'多单超出挂单金额--撤单'
                    self.log_text(msg_)
                self.buy(price=price + self.pricetick * slip, volume=buy_size, closetime=newTick.closetime)
                msg_ = f'开多仓---开仓价格:{price + self.pricetick * slip}---size:{buy_size}---time:{current_time}---symbol:{self.__code__}'
                self.log_text(msg_)
    ...
    def on_order_new(self, order: OrderData):
        ...
        self.log_text(
            f"[order info]  localid:{order.localid},code:{order.code},isBuy:{order.isBuy},totalQty:{order.totalQty},price:{order.price},isCanceled:{order.isCanceled}")
        return
