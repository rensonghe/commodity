...
def time_interval(data):
    ...
    end_time2 = datetime.datetime.strptime('14:45:59','%H:%M:%S').time()
    ...
    end_time4 = datetime.datetime.strptime('02:15:59','%H:%M:%S').time()
...
x = 0.001
vwapv = 'vwapv_5s'
data['return'] = np.log(data[vwapv]/data[vwapv].shift(1))
data['return'] = data['return'].shift(-1)
data['cum_return'] = np.cumsum(data['return'])
data['return_bar'] = np.log(data[vwapv]/data[vwapv].shift(bar))
data['return_bar'] = data['return_bar'].shift(-bar)
...
def classify(y):
    ...
    if y > x:
        ...
        return -1
print(data['return_bar'].apply(lambda x:classify(x)).value_counts())
print(len(data[data['return_bar'].apply(lambda x:classify(x))==-1])/len(data['return_bar'].apply(lambda x:classify(x))))
#%%
...
#%%
def target_label(data, loss=0.001, bar=15, threshold=0.001):
    data = data.reset_index()
    for i in range(0, len(data)-bar):
        cum = np.cumsum([data.loc[i + j, 'return'] for j in range(1, 15)])
        if data['return_bar'][i] >= threshold and sum(x <= -loss for x in cum) == 0:
            data.loc[i, 'target'] = 1
        elif data['return_bar'][i] <=-threshold and sum(x >= loss for x in cum) == 0:
            data.loc[i, 'target'] = 0
        else:
            data.loc[i, 'target'] = -1
    ...
    return data
#%%
train = target_label(train, loss=0.001, bar=bar, threshold=x)
...
train_set_ = train_set_.iloc[:,5:123] #65
train_target_ = train_["target"]

train_set = train[train_col]
train_set = train_set.iloc[:,5:123] #65
train_target = train["target"]

test_set = test[train_col]
test_set = test_set.iloc[:,5:123]
test_target = test["return_bar"]
...
#%%
test_ = test.copy()
test_ = target_label(test_, loss=0.001, bar=bar, threshold=x)
test_ = test_[~test_['target'].isin([-1])]
test_set_ = test_[train_col]
test_set_ = test_set_.iloc[:,5:123]
...
print(abs(df['return_bar']).describe())
del df
#%% first model
...
def lightgbm_model(X_train, X_train_target,X_train_, X_train_target_, X_test, X_test_target, X_test_, X_test_target_, LGB_BO):
    ...
    features = train_.iloc[:, 5:123].columns
...
# train_predict = train_predict.iloc[-20000:,:]
...
#     return ranks[-1]
# #
# final_predict['pctrank'] = final_predict['predict'].rolling(5000).apply(pctrank, engine='numba', raw=True)
...
# signal_1 = signal[signal['predict']>=np.percentile(y_pred_train[-30000:], 95)]
# signal_0 = signal[signal['predict']<=np.percentile(y_pred_train[-30000:], 5)]
...
print(signal_df.loc[:,['return_bar','predict']].corr())
...
signal_df_only = signal_df.loc[:,[vwapv, 'price','predict','return_bar','side']]
...
train_target = abs(train['return_bar']).apply(lambda x:abs_classify(x))
test_set = signal_df[train_col]
test_set = test_set.iloc[:,5:123]#65
test_target = abs(signal_df["return_bar"]).apply(lambda x:abs_classify(x))
print(signal_df['return_bar'].apply(lambda x:abs_classify(x)).value_counts())
print(len(signal_df[signal_df['return_bar'].apply(lambda x:abs_classify(x))==1])/len(signal_df['return_bar'].apply(lambda x:abs_classify(x))))
...
for out_threshold in out_threshold_set:
    ...
    final_df_['acc'] = np.where((abs(final_df_['return_bar'])>=x)&(final_df_['return_bar']*final_df_['side_']>0),1,0)
    print('--------------------'
    ...
    final_df_ = final_df_.loc[:,['datetime','closetime',vwapv,'price','predict','return_bar','side']]
    # final_df_ = final_df_.loc[:, ['datetime', 'closetime', vwapv, 'price', 'pctrank', 'target', 'side']]
    print(final_df_.loc[:,['return_bar','predict']].corr())
    ...
    final_df.to_csv(
        '/home/xianglake/songhe/future_backtest/{}/INE_{}_20230403_0603_{}bar_{}_ST2.0_20231016_filter_{}_{}_{}.csv'
        .format(future, future, bar, vwapv,out_threshold, x, dollar))
