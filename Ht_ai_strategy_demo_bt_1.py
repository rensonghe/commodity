...
class Ht_ai_strategy_demo_bt(CtaTemplate):
    ...
    def on_init_new(self):
        ...
        self.stra_sub_ticks(self.__code__)
        self.stra_get_bars(self.__code__, period='m1', count=1)
        self.__comminfo__ = self.stra_get_comminfo(self.__code__)
    ...
    def on_tick_new(self, newTick: TickData):
        ...
        if int(closetime_ / 1000) - int(self.kill_time / 1000) > 60 * 5:
            ...
            if self.get_position(self.__code__) > 0:
                ...
                if pf > 0.007:
                    ...
                    self.sell(price=newTick.bid_price_1, volume=self.get_position(self.__code__)*0.5,
                              closetime= closetime_ )
                    msg_ = f'多头止盈部分离场---平仓价格:{newTick.bid_price_1}---size:{self.get_position(self.__code__)*0.5}---time:{current_time}---symbol:{self.__code__}'
                    self.log_text(msg_)
                elif pf <= -0.009:
                    con1 = 1
                    self.cancel_all(self.__code__)
            ...
            if self.get_position(self.__code__) < 0:
                ...
                if pf > 0.007:
                    ...
                    self.cancel_all(self.__code__)
                    self.kill_time = closetime_ / 1000
                    self.buy(price=newTick.ask_price_1, volume=-self.get_position(self.__code__)*0.5,
                             closetime=closetime_)
                    msg_ = f'空头止盈部分离场---平仓价格:{newTick.ask_price_1}---size:{self.get_position(self.__code__) * 0.5}---time:{current_time}---symbol:{self.__code__}'
                    self.log_text(msg_)
                elif pf <= -0.009:
                    con1 = 1
        ...
        if match_signal is not None:
            ...
            # 持仓金额+挂单金额
            final_values = limit_orders_values + self.get_position_avgpx(self.__code__) * abs(curPos)
    ...
    def on_bar_new(self, newBar: BarData):
        ...
        # print(newBar.time,'------------')
        # self.log_text(
        #     f"on_tick_new:{newBar.price},closetime:{newBar.closetime},time:{newBar.time},type:{type(newBar.time)}")
        # 每五分钟判断一次
        if int(newBar.time / 1000) - int(self.kill_time / 1000) > 60 * 5:
            ...
            if self.get_position(self.__code__) > 0:
                ...
                if pf > 0.007:
                    ...
                    self.cancel_all(self.__code__)
                    self.kill_time = newBar.closetime / 1000
                    self.sell(price=newBar.close * (1 - self.place_rate), volume=self.get_position(self.__code__)*0.5,
                              closetime=newBar.time / 1000)
                    msg_ = f'多头止盈部分离场---平仓价格:{newBar.close * (1 + self.place_rate)}---size:{self.get_position(self.__code__)*0.5}---time:{newBar.time}---symbol:{self.__code__}'
                    self.log_text(msg_)
                elif pf <= -0.009:
                    con1 = 1
            ...
            if self.get_position(self.__code__) < 0:
                ...
                if pf > 0.007:
                    ...
                    self.cancel_all(self.__code__)
                    self.kill_time = newBar.closetime / 1000
                    self.buy(price=newBar.close * (1 + self.place_rate), volume=-self.get_position(self.__code__)*0.5,
                             closetime=newBar.time / 1000)
                    msg_ = f'空头止盈部分离场---平仓价格:{newBar.close * (1 - self.place_rate)}---size:{self.get_position(self.__code__) * 0.5}---time:{newBar.time}---symbol:{self.__code__}'
                    self.log_text(msg_)
                elif pf <= -0.009:
                    con1 = 1
                    # print('-------------空头止损离场-------------', '品种:',self.model_symbol)
